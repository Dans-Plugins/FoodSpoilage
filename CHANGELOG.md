...
- The expiry date kept in an item's persistent data could not be read back, because it is stored without a time of day and was parsed as though it had one. Every check fell back to the item's lore, so food whose lore carried no date — as with an empty `text.expiry-date-lore` — never spoiled, which left the previous entry's fix without effect. The stored date is now read, at the same time of day the lore fallback assumes.
